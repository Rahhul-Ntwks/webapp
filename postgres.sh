...
sudo -u postgres createuser --superuser "${POSTGRES_USER}" || true
sudo -u postgres createdb "$POSTGRES_DB" || true
sudo -u postgres psql -c "ALTER USER \"$POSTGRES_USER\" WITH ENCRYPTED PASSWORD '$POSTGRES_PASSWORD';" || true
sudo -u postgres psql -c "ALTER DATABASE \"$POSTGRES_DB\" OWNER TO \"$POSTGRES_USER\";" || true
